...
var X_Dom_Event_devicePixelRatio = window.devicePixelRatio || ( window.screen.deviceXDPI / window.screen.logicalXDPI ) || 1,
	 ...
	 */
	X_Dom_Event_coordinateSystemForElementFromPoint = ( X_UA.SafariMobile || X_UA.iOSWebView ) < 5 ? 'page' : 'client', // iOS4 以下は clientX が undeifned...
	
	X_Dom_Event_CANCEL_MOUSE = {},
	X_DomEvent;

if( !( X_UA.Trident || X_UA.TridentMobile ) || 9 <= ( X_UA.Trident || X_UA.TridentMobile ) ){
	X_DomEvent = function( e, xnode ){
		...
		if( e.pointerType ||
			...
			( ( X_UA.Trident || X_UA.TridentMobile ) === 10 && type === 'click' && ( e.pointerType = 'mouse' ) ) ){
			// PointerEvent;
		...
		if( pointerEventType = X_Event_toPointer[ originalType ] ){
			...
			if( touches = e.changedTouches ){
				...
				for( i = touches.length; i; ){
					...
					// Android 2+, iOS4+
					offset  = ( X_UA.SafariMobile || X_UA.iOSWebView ) < 5 ? xtarget.offset() : target.getBoundingClientRect();
					...
					//( X_UA.SafariMobile || X_UA.iOSWebView ) < 5 && console.log( pointerEventType + ':[' + events[ i ].pageX + ',' + events[ i ].pageY + ']' + events[ i ].pointerId );
				};
				...
				this.button        = btn = e.button !== undefined ? e.button : e.which !== undefined ? e.which - 1 : -1;
				this.buttons       = e.buttons !== undefined ? e.buttons : btn === 0 ? 1 : btn === 1 ? 2 : btn === 2 ? 4 : 0;
				this.pressure      = ( btn !== -1 ? 0.5 : 0 );
			...
			// Opera で button==2の場合、コンテキストメニューイベントを発火 「ツール」->「設定」->「詳細設定」->「コンテンツ」->「Javascriptオプション」で「右クリックを制御するスクリプトを許可する」
				if( originalType === 'mousedown' && btn === 2 && ( X_UA.Presto || X_UA.PrestoMobile ) ){
			...
			this.button        = btn = e.button !== undefined ? e.button :
	...
	X_DomEvent = function( e, xnode, element ){
		...
		this.currentTarget = xnode; // xnode
		this.relatedTarget = X_Node_getXNode( e.fromElement || e.toElement ); // xnode
...
if( document.onwheel === undefined ){
	// DOMMoseScroll
	if( ( X_UA.Gecko || X_UA.Fennec ) && window.MouseScrollEvent ){
		if( 0 <= X_Number_conpareVersion( X_UA.ENGINE_VERSION, '1.9.1' ) ){ // Gecko 1.9.1+ (firefox3.5+)
			//console.log( 'wheel <= MozMousePixelScroll' );
			X_Event_Rename.wheel = 'MozMousePixelScroll';
		} else
		if( 0 <= X_Number_conpareVersion( X_UA.ENGINE_VERSION, '0.9.7' ) ){ // Gecko 0.9.7+ (NN6.2+?)
...
if( X_elmHtml && X_elmHtml.style.msAnimation !== undefined && X_elmHtml.style.animation === undefined ){ //document.documentElement.style.msAnimation 
...
if( !navigator.pointerEnabled ){
	...
	if( X_UA_HID.TOUCH ){
		...
		( X_UA.Presto || X_UA.PrestoMobile ) && ( X_Event_Rename.contextmenu  = 'mousedown' );
