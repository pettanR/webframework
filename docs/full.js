...
(function(undefined) {
  ...
  (function() {
    ...
    var ua = X_UA, html = document.documentElement, dua = navigator.userAgent, dav = navigator.appVersion, tv = parseFloat(dav) | 0, sys = navigator.platform, docMode = document.documentMode, screenW = screen.width, screenH = screen.height, isTouch = window.ontouchstart !== undefined, NINTENDO_ = "Nintendo ", WIN_PHONE = "Windows Phone", ANDROID = "Android", ANDROID_ = ANDROID + " ", PC_MODE = "PCMode", VERSION_ = "Version/", I_PHONE = "iPhone", LINUX = "Linux", SAFARI = "Safari", NETSCAPE = "Netscape", 
    verVersion = getNumber(dav, VERSION_) || getNumber(dua, VERSION_), isPrsto = window.opera, verOpera = isPrsto && (isPrsto.version ? parseFloat(isPrsto.version()) : Math.max(getNumber(dua, "Opera"), verVersion, tv)), isOPR = window["opr"], isOpMin = window["operamini"], verOpMin = isOpMin && Math.max(getNumber(dua, "Opera Mini/"), getNumber(dua, "Opera Mobi/"), verVersion), isUCWEB = findString(dua, "UCWEB"), verUC2 = getNumber(dua, " U2/"), isTrident = !isPrsto && (document.all || docMode), isEdge = 
    !isTrident && html["msContentZoomFactor"], isBlink = !isEdge && window.chrome, isSafari = findString(dua, SAFARI), isIris = findString(dua.toLowerCase(), "iris"), isGecko = html && html.style["MozAppearance"] !== undefined, isKHTML = findString(dav, "Konqueror"), isYahooAdr = findString(dav, "YJApp-ANDROID"), isEdgeAdr = findString(dav, "EdgA/"), PS3 = getNumber(dua.toUpperCase(), "PLAYSTATION 3"), PSP = findString(dav, "PSP"), PSVita = getNumber(dua, "PlayStation Vita"), NDS = sys === "Nitro", 
    NDSi = sys === NINTENDO_ + "DSi", N3DS = sys === NINTENDO_ + "3DS", New3DS = sys === "New " + NINTENDO_ + "3DS" || findString(dua, I_PHONE + " OS 6_0") && screenW === 320 && screenH === 240, Wii = sys === NINTENDO_ + "Wii", WiiU = sys === NINTENDO_ + "WiiU", iOS = !New3DS && fromString(sys, "iP") || fromString(dua, "; iPh OS "), WebOS = window["palmGetResource"], WinPhone = getNumber(dua, WIN_PHONE) || getNumber(dav, WIN_PHONE + " OS ") || getNumber(dua, "; wds"), wpPCMode = findString(dav, "ZuneWP"), 
    Win = fromString(sys, "Win"), Mac = fromString(sys, "Mac"), Kobo = findString(dua, "Kobo"), Kindle = findString(dua, "Kindle"), SonyReader = findString(dua, "EBRD"), Mylo = tv === 2 && findString(dua, "Sony/COM2/"), Android = findString(sys, ANDROID) || isGecko && findString(dav, ANDROID) || isYahooAdr, Linux = findString(sys, LINUX), MeeGo = findString(dua, "MeeGo") && findString(dua, "NokiaBrowser/8.5.0"), XBoxOne = findString(dua, "Xbox One"), XBox360 = !XBoxOne && findString(dua, "Xbox"), 
    FireFoxOS, BlackBerry, Solaris, verAndroid = getNumber(sys, ANDROID_) || getNumber(dav, ANDROID_) || getNumber(dua, ANDROID_) || getNumber(dua, "; Adr "), verSafari = verVersion, verTrident = getNumber(dav, "Trident/"), verEdge = getNumber(dav, "Edge/"), verMSIE = docMode ? docMode : window.XMLHTTPRequest ? document.getElementsByTagName ? 7 : 4 : document.compatMode ? 6 : (0).toFixed ? 5.5 : window.attachEvent ? 5 : 4, verGecko = getNumber(dua, "rv:"), verWebKit = getNumber(dua, "AppleWebKit/"), 
    verChrome = getNumber(dua, "Chrome/"), verOPR = getNumber(dua, "OPR/"), verFennec = getNumber(dua, "Fennec/"), verNetscape = getNumber(dua, NETSCAPE + "6/") || getNumber(dua, NETSCAPE + "/") || getNumber(dua, "Navigator/"), verNetFront = getNumber(dua, "NetFront/"), ver_iCab = getNumber(dua, "iCab"), maybeAOSP = isBlink && verWebKit <= 534.3, maybePCMode = isTouch && (verWebKit || isGecko) && (sys === LINUX + " armv7l" || sys === LINUX + " i686") && findString(dua, LINUX + " x86_64") || !verAndroid && 
    isYahooAdr, v, pcMode, dpRatio;
    if (Kobo) {
      ua["Kobo"] = true;
    } else {
      if (Kindle) {
                                  ...
                                  if (WebOS) {
                                    ...
                                    if (MeeGo) {
                                      ...
                                      if (iOS) {
                                        ...
                                        if (!v) {
                                          ua[PC_MODE] = true;
                                          v = window["WebAssembly"] ? 11.2 : window["HTMLMeterElement"] ? 10.3 : window["Proxy"] ? 10.2 : window["HTMLPictureElement"] ? 9.3 : Number["isNaN"] ? 9.2 : window["SharedWorker"] ? window["performance"] && window["performance"]["now"] ? 8 : 8.4 : document.execCommand ? 7.1 : window["webkitURL"] ? 6.1 : window["Worker"] ? 5.1 : window["Int8Array"] ? 4.3 : window["HTMLAudioElement"] ? 4.1 : 3.2;
                                        }
                                        ua["iOS"] = v;
                                                        ...
                                                        if (Linux && maybePCMode) {
                                                          ...
                                                          if (isBlink && !maybeAOSP || isOPR || verOPR) {
                                                            ...
                                                            if (document["registerElement"]) {
                                                              ...
                                                              if (window["Int8Array"]) {
                                                                ua[ANDROID] = verAndroid = !navigator["connection"] ? 4.4 : !window["searchBoxJavaBridge_"] && !isBlink ? 4.2 : Number.isNaN ? 4.1 : 4;
                                                              } else {
                                                                ua[ANDROID] = verAndroid = verWebKit < 529 ? 1.5 : verWebKit < 531 ? 2 : verWebKit < 534 ? window.HTMLAudioElement ? 2.3 : 2.2 : 3;
                                                              }
                                                            }
                                                          }
                                                        } else {
                                      ...
                                      if (isIris) {
                                        ...
                                        if (isSafari) {
                                          if (verSafari) {
                                            ...
                                            if (verWebKit <= 528.16) {
                                              ua[SAFARI] = verWebKit < 73 ? .8 : verWebKit < 85 ? .9 : verWebKit < 100 ? 1 : verWebKit < 125 ? 1.1 : verWebKit < 312 ? 1.2 : verWebKit < 412 ? 1.3 : verWebKit <= 419.3 ? 2 : verWebKit <= 525.13 ? 3 : verWebKit <= 525.25 ? 3.1 : 3.2;
                                            }
                                          }
                                        }
                                      }
  ...
  var X_Script_VBS_ENABLED = X_UA["Windows"] && !X_UA["WinCE"] && !X_UA["WinPhone"] && X_UA["IE"] < 11 && X_UA["IEHost"] !== 11, X_Script_gte15 = !(X_UA["IE"] < 5.5) && new Function("f,a", "try{return f.apply({},a)}catch(e){}");
  ...
  if (X_UA["IE"] === 6 && !X_Script_try(function() {
  ...
  var __ClassBase__ = {NAME:""}, X_Class_CLASS_LIST = [], X_Class_DEF_LIST = [], X_Class_SUPER_CALLER = [], X_Class_SUPER_STACKS = [], X_Class_traits = null, X_Class_useObjectCreate = false, X_Class_use_proto_ = !(X_UA["Prsto"] && X_UA["Android"]) && !X_UA["AOSP"] && !X_UA["CrWV"] && !!X_emptyFunction.prototype.__proto__, X_Class_constructorFix = X_UA["AOSP"] < 3 || X_UA["iOS"] < 5, X_Class_SEAL_KILLING = [], X_Class_CommonMethods = {"kill":function() {
  ...
  if (!FocusUtility_docActiveElmSupport) {
    if (document.addEventListener) {
      ...
      document.addEventListener("blur", function(event) {
        ...
        if (tgt === FocusUtility_fixActiveElm) {
          FocusUtility_fixActiveElm = null;
        }
      }, false);
    } else {
      if (document.attachEvent) {
  ...
  function FocusUtility_getFocusedElement() {
    ...
    return X_Script_gte15 ? X_Script_try(X_Object_find, [document, "activeElement"]) : window["vbs_testAE"]() && document.activeElement;
  }
  ...
  var X_System_postMessageAccessKey = window.postMessage ? Math.random() * 1E4 | 0 : 0, X_System_MESSAGE_RECIVERS = X_System_postMessageAccessKey && {}, X_System = X_Class_override(X_EventDispatcher(), {monitor:function() {
  }, gc:function() {
  }, message:function(name, callback) {
    var key = Math.random() * 1E4 | 0;
  ...
  X["ViewPort"] = {"listen":function(type, arg1, arg2, arg3) {
    ...
    var elm, scrollY, docH, winH, _scrollY;
    if (X_UA["iOS"]) {
  ...
  } : window.scrollX !== undefined ? function() {
    X_Node_updateTimerID && X_Node_startUpdate();
    return[X_ViewPort_scrollX = window.scrollX, X_ViewPort_scrollY = window.scrollY];
  } : function() {
    X_Node_updateTimerID && X_Node_startUpdate();
    ...
    X_DomEvent = function(e, xnode) {
      ...
      if (e.pointerType || X_UA["IE"] === 10 && type === "click" && (e.pointerType = "mouse")) {
        if (X_Dom_Event_convertMSPointerType) {
          ...
          this["pressure"] = isNum(e.pressure) ? e.pressure : e.button !== -1 ? .5 : 0;
        ...
        if (pointerEventType = X_Event_toPointer[originalType]) {
          if (touches = e.changedTouches) {
            ...
            shiftKey = e.shiftKey;
            time = X_Timer_now();
            force = originalType === "touchend" || originalType === "touchcancel" ? 0 : .5;
            ...
            this["pressure"] = this["button"] !== -1 ? .5 : 0;
            elm = e.target;
  ...
  if (X_UA["Opera"] || X_UA["Webkit"] || X_UA["Blink"] || X_UA["Khtml"] || X_UA["AOSP"] || X_UA["CrWV"]) {
      ...
      if (1 < arguments.length) {
        ...
        if (X_Type_isString(nameOrObj)) {
          ...
          switch(nameOrObj) {
            ...
            ;
            case "value":
              if (tag === "INPUT" && X_Node_Attr_STATIC_VALUE_TYPES[attrs["type"]]) {
                break;
              }
            ;
            case "checked":
  ...
  function X_Node_css(nameOrObj) {
    var args = arguments, css = this["_css"], bad = !this["_tag"] || X_Dom_DTD_MOVE_TO_HEAD[this["_tag"]] || this["_tag"] === "SCRIPT", p, name, v, plain, camelize, flags, changed;
  ...
  function X_NodeAnime_stopNow(xnode) {
    ...
    if (flags & ~X_Node_BitMask_RESET_GPU) {
      ...
      flags & X_NodeFlags_GPU_RELEASE_RESERVED || X_NodeAnime_updatePosition(xnode, obj, .5, false);
  ...
  var X_NodeAnime_ease = {"quadratic":function(k) {
    ...
    if (k < 1 / 2.75) {
      ...
      if (k < 2 / 2.75) {
        return X * (k -= 1.5 / 2.75) * k + .75;
      } else {
        if (k < 2.5 / 2.75) {
          return X * (k -= 2.25 / 2.75) * k + .9375;
        } else {
          return X * (k -= 2.625 / 2.75) * k + .984375;
        }
      }
    }
  }, "elastic":function(k) {
    return k === 0 ? 0 : k === 1 ? 1 : .4 * Math.pow(2, -10 * k) * Math.sin((k - .055) * 28.56) + 1;
  }};
  ...
  function X_Node_createText(text) {
    ...
    this["append"](text = X_Doc_createText(text));
    return text;
  }
  function X_Node_createTextAt(index, text) {
    ...
    this["appendAt"](index, text = X_Doc_createText(text));
    return text;
  }
  ...
  }, X_XHR_xdr = X_XHR_createXDR && X_XHR_createXDR(), X_XHR_msXMLVer = 0, X_XHR_msXMLName = "", X_XHR_msXML, X_XHR_createMSXML = X_UA["ActiveX"] && (5 <= X_UA["IE"] && X_UA["IE"] < 7 || 7 <= X_UA["IE"] && X_UA["IE"] < 9 && X_URL_IS_LOCAL) && function() {
  ...
  if (X_XHR_w3c || X_XHR_msXML) {
    ...
    X_TEMP.X_XHR_params = {"_rawType":X_EventDispatcher_EVENT_TARGET_XHR, _isXDR:false, _isMsXML:false, _method:"", _dataType:"", _busy:false, _canceled:false, _error:false, _percent:0, _timerID:0, load:function(obj) {
      var raw = X_XHR["_rawObject"], method = obj["method"], url = obj["url"], async = obj["async"] !== false, username = obj["username"], password = obj["password"], headers = obj["headers"] || {}, postdata = obj["postdata"] || "", timeout = obj["timeout"] || 2E4, noCache = obj["cache"] !== true, dataType = X_XHR._dataType = obj["dataType"], xDomain = !X_URL_isSameDomain(url), isFile = X_URL_isLocal(url), init, type, tmp, p;
  ...
  X_TEMP.X_JSONP_cb = function(accessKey, jsonString, time, opt_json2FileSize) {
    ...
    console.log("ms : " + time + " speed : " + (jsonString.length + (opt_json2FileSize || 0)) / time * 1E3 + " \u30d0\u30a4\u30c8/\u79d2.");
  };
  ...
  function X_JSONP_iframeListener(e) {
    switch(e.type) {
      case "ninjaload":
        ...
        X_JSONP_errorTimerID = X_JSONP["asyncDispatch"](1E3, X_EVENT_ERROR);
        break;
      case "ninjaerror":
  ...
  X_TEMP.X_FormSender_params = {_busy:false, _canceled:false, load:function(option) {
    ...
    target = target === "_self" ? "_parent" : target === "_blank" ? "_self" : target || "_self", html = ['<form method="', X_String_toChrReferanceForHtmlSafety(option["method"] || "GET"), '" action="', X_String_toChrReferanceForHtmlSafety(url || ""), '" target="', X_String_toChrReferanceForHtmlSafety(target), '">'];
    X_FormSender_isLeave = target === "_top" || target === "_parent";
    X_FormSender_isSameDomain = X_URL_isSameDomain(url);
    for (k in params) {
      html.push('<input type="hidden" name="', X_String_toChrReferanceForHtmlSafety(k), '" value="', X_String_toChrReferanceForHtmlSafety(params[k] || ""), '">');
    }
  ...
  var X_GadgetXHR_canUse = 5.5 <= X_UA["IE"] || !X_UA["IE"], X_GadgetXHR_iframeName = "gadgetProxy_" + (Math.random() * 1E5 | 0), X_GadgetXHR_GADGET_XML_URL = "http://pettanr.github.io/webframework/gadget/X.Net.XHRGadget.xml", X_GadgetXHR_GADGET_URL = "http://www.ig.gmodules.com/gadgets/ifr?url=" + encodeURIComponent(X_GadgetXHR_GADGET_XML_URL) + "&nocache=1", X_GadgetXHR_IMAGE_URL = "img/opacity0.gif", X_GadgetXHR_maxQueryLength = X_UA["IE"] ? 2E3 : 6E3, X_GadgetXHR_requestBatches, X_GadgetXHR_requestOriginal, 
  ...
  X["OAuth2"] = X_EventDispatcher["inherits"]("X.OAuth2", X_Class.NONE, {"Constructor":function(obj) {
    ...
    obj["refreshMargin"] = obj["refreshMargin"] || 3E5;
  ...
  function X_OAuth2_responceHandler(e) {
    ...
    switch(e.type) {
      case X_EVENT_SUCCESS:
        ...
        if (data["expires_in"]) {
          X_OAuth2_setAccessTokenExpiry(this, X_Timer_now() + data["expires_in"] * 1E3);
        } else {
  ...
  X["Audio"] = X_EventDispatcher["inherits"]("X.Audio", X_Class.NONE, {"source":"", "backendName":"", "Constructor":function(sourceList, opt_option) {
    ...
    var pair = X_Pair_get(this);
    if (obj === undefined) {
      return pair ? pair.getState() : {"startTime":-1, "endTime":-1, "loopStartTime":-1, "loopEndTime":-1, "currentTime":-1, "loop":false, "looded":false, "error":0, "autoplay":false, "playing":false, "source":this["source"], "duration":0, "volume":.5};
    }
  ...
  var X_WebAudio_Context = (!X_UA["iOS"] || X_UA["iPhone"] === "5+" || X_UA["iPad"] === "3+|2min+" || X_UA["iPod"] === "5+") && !(X_UA["Fennec"] && !(4 <= X_UA["Android"])) && !X_UA["AOSP"] && !(X_UA["CrWV"] < 5) && !(40 <= X_UA["Gecko"] && X_UA["Windows"]) && !X_UA["Edge"] && (window["AudioContext"] || window["webkitAudioContext"] || window["mozAudioContext"]), X_WebAudio_context, X_WebAudio_BUFFER_LIST = [], X_WebAudio_need1stTouch = X_UA["iOS"], X_WebAudio_isNoTouch = X_WebAudio_need1stTouch, 
  X_WebAudio_needRateFix = X_WebAudio_need1stTouch, X_WebAudio, X_WebAudio_BufferLoader, X_WebAudio_fpsFix;
  if (X_WebAudio_Context) {
    ...
    X_WebAudio = X_AudioBase["inherits"]("X.WebAudio", X_Class.POOL_OBJECT, {loader:null, _startPos:0, _endPosition:0, _startTime:0, _interval:0, audioBuffer:null, bufferSource:null, gainNode:null, bufferPlay:"", bufferStop:"", "Constructor":function(dispatcher, url, option) {
      ...
      this._startTime = X_WebAudio_context.currentTime * 1E3;
      ...
      this.bufferSource[this.bufferPlay](0, begin / 1E3, (end - begin) / 1E3);
      ...
      var time;
      if (this.playing) {
        time = X_WebAudio_context.currentTime * 1E3 - this._startTime - this._endPosition + this._startPos | 0;
        if (time < 0) {
      ...
      return X_WebAudio_context.currentTime * 1E3 - this._startTime + this._startPos | 0;
  ...
  var X_HTMLAudio, X_HTMLAudio_seekingFixIOS = 7 <= X_UA["iOS"], X_HTMLAudio_endedFixIOS = X_UA["iOS"] < 7, X_HTMLAudio_endedFixAOSP2 = X_UA["AOSP"] < 3, X_HTMLAudio_endedFixAOSP3 = !X_HTMLAudio_endedFixAOSP2 && X_UA["AOSP"] < 4, X_HTMLAudio_endedFixAOSP4 = 4 <= X_UA["AOSP"], X_HTMLAudio_endedFixCWV = X_UA["CrWV"] || X_UA["Blink"] && X_UA["Android"], X_HTMLAudio_currentTimeFix = X_UA["Prsto"] && X_UA["Android"], X_HTMLAudio_playStartFix = X_UA["Windows"] && 44 <= X_UA["Gecko"], X_HTMLAudio_volumeFix = 
  X_UA["Chrome"], X_HTMLAudio_volumeEnabled = X_UA["WinPhone"] !== 7.5 && !X_UA["Prsto"], X_HTMLAudio_needPlayForSeek = X_UA["iOS"] || X_UA["Gecko"], X_HTMLAudio_pauseFix = 12 <= X_UA["Prsto"] && 0 < " XP XPSP2 2003|XP64".indexOf(X_UA["Windows"]), X_HTMLAudio_need1stTouch = X_UA["iOS"] || 4.2 <= X_UA["AOSP"] || X_UA["CrWV"] || X_UA["WinPhone"] || X_UA["Blink"] && X_UA["Android"], X_HTMLAudio_playTrigger = X_UA["WinPhone"] === 7.5 ? "canplay" : X_UA["iOS"] < 8 ? "suspend" : X_UA["iOS"] ? "loadedmetadata" : 
  X_UA["Blink"] < 32 ? "stalled" : "canplaythrough", X_HTMLAudio_durationFix = X_UA["iOS"] < 8 || X_UA["CrWV"] || X_UA["WinPhone"] === 7.5 || X_UA["Windows"] && 12 <= X_UA["Prsto"] || X_UA["Blink"] < 36 && X_UA["Android"], X_HTMLAudio_shortPlayFix = X_UA["AOSP"], X_HTMLAudio_progressEnabled = !(X_UA["Prsto"] && X_UA["Android"]) && X_UA["WinPhone"] !== 7.5;
  if (X_Audio_constructor) {
    X_HTMLAudio = X_AudioBase["inherits"]("X.HTMLAudio", X_Class.POOL_OBJECT, {_readyState:0, _src:"", _touchState:X_HTMLAudio_need1stTouch ? 1 : 0, _currentFixStart:0, _currentFixBegin:0, _durationFixPhase:X_HTMLAudio_durationFix ? 1 : 0, _lastCurrentTime:0, _shortPlayFixON:false, _shortPlayFixTime:0, _endedFixON:false, _seekingFixON:false, "Constructor":function(dispatcher, source, option) {
      ...
      switch(e.type) {
        ...
        case "progress":
          if (X_HTMLAudio_progressEnabled && this.duration && this._readyState < 3) {
            ...
            this.dispatcher["dispatch"]({type:X_EVENT_PROGRESS, "percent":time * 1E3 / this.duration * 100});
          }
          break;
        case "loadeddata":
        ;
        ...
        ;
        case "durationchange":
          if (!this.duration || this.duration !== raw.duration * 1E3) {
                  ...
                  if ((now = this.getActualCurrentTime()) === this._lastCurrentTime) {
                    ...
                    if (X_HTMLAudio_playStartFix && now < this._lastCurrentTime) {
                      ...
                      if (this.playing) {
                        ...
                        if (0 + end <= 0 + now || now < this._lastCurrentTime && now < 2E3) {
                          if (this.autoLoop) {
      ...
      if (this._shortPlayFixON) {
        this._shortPlayFixTime = 1E3 < end - begin ? 200 : 400;
      ...
      if (this._endedFixON) {
        ...
        if (!(this._durationFixPhase % 8) && this.duration) {
          raw.currentTime = this._lastCurrentTime / 1E3;
        }
      ...
      return X_HTMLAudio_currentTimeFix ? X_Timer_now() - this._currentFixStart + this._currentFixBegin : this._seekingFixON ? this._lastCurrentTime : this["_rawObject"].currentTime * 1E3 | 0;
    ...
    X_SLAudio = X_AudioBase["inherits"]("X.SilverlightAudio", X_Class.POOL_OBJECT, {"_rawType":X_EventDispatcher_EVENT_TARGET_SILVER_LIGHT, _onload:"", _callback:null, xnodeObject:null, _source:"", _ended:true, _paused:false, _lastUserAction:"", _lastState:"", _interval:0, "Constructor":function(dispatcher, source, option) {
      ...
      switch(e.type) {
        case "MediaFailed":
          ...
          break;
        case "MediaOpened":
          this.duration = this["_rawObject"]["NaturalDuration"]["Seconds"] * 1E3;
          this.dispatcher["asyncDispatch"](X_EVENT_READY);
          break;
        case "MediaEnded":
          this._ended = true;
      ...
      begin = (begin / 1E3 | 0) * 1E3 + (500 < begin % 1E3 ? 1E3 : 0);
      this["_rawObject"]["Volume"] = this.gain;
      ...
      if (!this._interval) {
        this._interval = X_Timer_add(1E3, 0, this, this._onInterval);
      }
    }, _onInterval:function() {
      if (!this.playing) {
        delete this._interval;
      ...
      return this["_rawObject"]["Position"]["Seconds"] * 1E3 | 0;
    }, afterUpdateState:function(result) {
      var end, halfway;
      if (result & 3) {
        this.actualPlay();
  ...
  if (X_Plugin_WMP_VERSION) {
    X_WMPAudio = X_AudioBase["inherits"]("X.WMPAudio", X_Class.POOL_OBJECT, {xnodeObject:null, wmp:null, _wmp:null, _readyState:0, _seekDirection:0, _timerID:0, "Constructor":function(dispatcher, source, option) {
      ...
      this.wmp["CurrentPosition"] = begin / 1E3;
      if (!this._timerID) {
        this._timerID = X_Timer_add(100, 0, this, this._onTimer);
      }
    }, _onTimer:function() {
      ...
      if (this._readyState === 1) {
        ...
        if (progress < 100) {
          ...
          this._readyState = 2;
          if (7 <= X_Plugin_WMP_VERSION) {
            this.duration = this._wmp["currentMedia"].duration * 1E3 | 0;
          } else {
            this.duration = this.wmp["Duration"] * 1E3 | 0;
          }
      ...
      if (7 <= X_Plugin_WMP_VERSION) {
        ...
        this.wmp["Volume"] = (1 - this.gain) * 1E4;
      }
    }, getActualCurrentTime:function() {
      return this.wmp["CurrentPosition"] * 1E3 | 0;
    }, afterUpdateState:function(result) {
      if (result & 3) {
        this.actualPlay();
      } else {
  ...
  var X_AudioSprite_shouldUse = X_HTMLAudio && (X_UA["iOS"] || X_UA["AOSP"] || X_UA["Prsto"] && X_UA["Android"]), X_AudioSprite_useVideoForMulti = false, X_AudioSprite_disableMultiTrack = !X_WebAudio && (X_UA["iOS"] || 4 <= X_UA["AOSP"] || X_UA["CrWV"] || X_UA["WinPhone"] === 7.5), X_AudioSprite_enableVolume = X_HTMLAudio && !X_UA["iOS"] && !X_UA["AOSP"] && !(X_UA["Prsto"] && X_UA["Android"]) && !(X_UA["Fennec"] < 25), X_AudioSprite_maxTracks = X_AudioSprite_useVideoForMulti ? 2 : X_AudioSprite_disableMultiTrack ? 
  1 : 9, X_AudioSprite_lengthSilence = 1E4, X_AudioSprite_lengthDistance = 5E3, X_AudioSprite_uid = 0, X_AudioSprite_TEMP = {presets:{}, BGMs:{}, tracks:[], pauseTracks:[], volume:1, bgmTrack:null, bgmPosition:0, bgmName:"", bgmLooped:false, bgmPlaying:false, tmpEvent:null}, X_AudioSprite, X_AudioSprite_numTracks, X_AudioSprite_useVideo;
  ...
  X_TextRange = function(xnode, rangeType, a, b) {
    var tag = xnode["_tag"], type = xnode["_attrs"] && xnode["_attrs"]["type"], flags = xnode["_flags"], elm = xnode["_rawObject"], isTextField = tag === "TEXTAREA" || tag === "INPUT" && 0 < " text,search,url,telephone,password".indexOf(type);
    ...
    if (flags & X_NodeFlags_IN_TREE && !(flags & X_NodeFlags_STYLE_IS_DISPLAY_NONE)) {
      ...
      if (rangeType === "selection") {
        if (isTextField && FocusUtility_getFocusedElement() !== elm) {
          return;
        }
      ...
      if (isTextField && !X_TextRange_ieRange) {
        return;
      }
      switch(rangeType) {
        case "point":
          if (!X_Type_isFinite(a + b)) {
            return;
          }
  ...
  function X_TextRange_getCorrectRect(isTextField, range) {
    var rect, l, endOffset, endContainer, _rect, left, height, right, _left, _height;
    if (!isTextField && X_TextRange_w3cRange) {
      if (!X_UA["IE"] && !X_UA["Edge"] && !X_UA["iOS"]) {
        ...
        if (X_UA["iOS"]) {
          ...
          if (range.collapsed) {
            ...
            l = endContainer.data.length;
            if (l) {
              if (range.endOffset === l) {
                range.startOffset -= 1;
                _rect = range.getBoundingClientRect();
                rect = {left:_rect.right, top:_rect.top, right:_rect.right, bottom:_rect.bottom, width:0, height:_rect.height};
                range.startOffset += 1;
              } else {
                range.endOffset += 1;
                _rect = range.getBoundingClientRect();
                rect = {left:_rect.left, top:_rect.top, right:_rect.left, bottom:_rect.bottom, width:0, height:_rect.height};
                range.endOffset -= 1;
              }
            } else {
              rect = endContainer.getBoundingClientRect();
            }
          }
          return rect;
        } else {
          ...
          if (!range.collapsed) {
            ...
            if (_rect.height < rect.height) {
              rect = {left:_rect.left, top:rect.top, right:rect.right, bottom:_rect.bottom, width:rect.right - _rect.left, height:_rect.height};
            }
      ...
      if (range.text) {
        ...
        if (left < _left || _height < height) {
          rect = {left:_left, top:range.boundingTop, right:right, bottom:_height + range.boundingTop, width:right - _left, height:_height};
          range.moveEnd("character", 1);
          if (X_UA["IE"] < 8) {
            if (rect.width === 0) {
          ...
          range.moveEnd("character", 1);
          rect = {left:left, top:range.boundingTop, right:right, bottom:height + range.boundingTop, width:range.boundingWidth, height:height};
        }
      }
      return rect;
    }
  ...
  function X_TextRange_getOffset() {
    var result = this._result, range, _range, from;
  ...
  function X_TextRange_getRect() {
    var result = this._result, rect = result.rect;
    if (!rect) {
      rect = result.rect = X_TextRange_getCorrectRect(this._isTextField, result.range);
    }
    if (!this._isTextField && X_TextRange_w3cRange) {
      return{"x":rect.left | 0, "y":rect.top | 0, "width":(rect.width || rect.right - rect.left) | 0, "height":(rect.height || rect.bottom - rect.top) | 0};
    } else {
      return{"x":rect.left, "y":rect.top, "width":rect.width, "height":rect.height};
    }
  }
  function X_TextRange_move(from, to) {
    var result = this._result, range = result.range, elm = this._xnode["_rawObject"], texts, i, z, n, text, len, start, end, _range;
  ...
  function X_TextRange_text(v) {
    var result = this._result, range = result.range, from = result.from, to = result.to, attrs, range, elm, val;
  ...
  function X_TextRange_select() {
    var result = this._result, range, elm;
    if (!this._isTextField) {
      if (X_TextRange_w3cRange) {
        X_TextRange_w3cSelection.removeAllRanges();
  ...
  function X_TextRange_lazyFocus(result) {
    var range = result.range, selection = X_TextRange_ieSelection.createRange(), elm = this["_rawObject"];
    if (FocusUtility_getFocusedElement() !== elm) {
      elm.focus();
    }
  ...
  RangeFromSelection = function(xnode, isTextField, truncate) {
    ...
    return result && {_xnode:xnode, _isTextField:isTextField, _result:result, "getOffset":isTextField ? RangeFromSelection_getOffsetTextField : X_TextRange_w3cRange ? RangeFromSelection_getOffsetW3c : X_TextRange_getOffset, "getRect":!isTextField || X_TextRange_ieRange ? X_TextRange_getRect : null, "move":X_TextRange_move, "text":isTextField ? X_TextRange_text : null, "select":X_TextRange_select};
  };
  function RangeFromSelection_getRawRange(xnode, isTextField, truncate) {
    var elm = xnode["_rawObject"], selection, i, l, range, anchor, focus, parent, _range;
    if (isTextField && X_TextRange_canGetCursorPosition && !X_TextRange_ieRange) {
      ...
      if (!isTextField && X_TextRange_w3cRange) {
        ...
        if (range) {
          return{range:range, rect:X_TextRange_getCorrectRect(isTextField, range)};
        }
      } else {
  ...
  function RangeFromSelection_getOffsetTextField() {
    var result = this._result, xnode = this._xnode, elm = xnode["_rawObject"], l, from, to, ret, selection_range, before_range, text;
    if (X_TextRange_canGetCursorPosition) {
      if (X_UA["IE"]) {
        l = elm.value.length;
        from = elm.selectionStart;
    ...
    return{"from":result.from = from, "to":result.to = to};
  }
  function RangeFromSelection_getOffsetW3c() {
    var result = this._result, range = result.range, texts = [], startText = range.startContainer, i = -1, n = range.startOffset, text, from, to;
  ...
  RangeFromIndex = function(xnode, isTextField, from, to) {
    ...
    return{_xnode:xnode, _isTextField:isTextField, _result:{range:range, from:0, to:0}, "getOffset":X_TextRange_getOffset, "getRect":!isTextField || X_TextRange_ieRange ? X_TextRange_getRect : null, "move":X_TextRange_move, "text":isTextField ? X_TextRange_text : null, "select":X_TextRange_select}["move"](from, to);
  };
  "use strict";
  RangeFromPoint = function(xnode, isTextField, x, y) {
    var result = RangeFromPoint_getRawRange(xnode, isTextField, x, y);
    return result && {_xnode:xnode, _isTextField:isTextField, _result:result, "getOffset":X_TextRange_getOffset, "getRect":!isTextField || X_TextRange_ieRange ? X_TextRange_getRect : null, "move":X_TextRange_move, "text":isTextField ? X_TextRange_text : null, "select":X_TextRange_select};
  };
  function RangeFromPoint_getRawRange(xnode, isTextField, x, y) {
    var range = null, texts, range, i, offset, l, text, j, m, elm, parent, rect, top, left;
    if (!isTextField && X_TextRange_w3cRange) {
      ...
      for (i = offset = 0, l = texts.length;text = texts[i];++i) {
        ...
        if (X_TextRange_hitRect(rect, x, y)) {
          for (j = 0;j < m;++j) {
            ...
            rect = X_TextRange_getCorrectRect(isTextField, range);
            if (X_TextRange_hitRect(rect, x, y)) {
              return{range:range, rect:rect, from:offset + j, to:offset + j + 1};
            }
          }
        }
        offset += m;
  ...
  LineRangeFromIndex = function(xnode, isTextField, from, to) {
    ...
    return result && {_xnode:xnode, _isTextField:isTextField, _result:result, _getRawRange:LineRangeFromIndex_getRawRange, "getOffset":X_TextRange_getOffset, "getRect":!isTextField || X_TextRange_ieRange ? X_TextRange_getRect : null, "move":X_TextRange_move, "text":isTextField ? X_TextRange_text : null, "select":X_TextRange_select};
  };
  function LineRangeFromIndex_getRawRange(range, xnode, isTextField, from, to) {
    var elm = xnode["_rawObject"], texts, i, l, currentNode, isLastNode, j, m, isLast, rect, maxB, lines = 0, lastTextNode, lastOffset, totalOffset = 0, startLineTextNode, startLineOffset, startLineFrom, currentLineStartTextNode, currentLineStartOffset, currentLineStartFrom, _range;
    if (!isTextField && X_TextRange_w3cRange) {
      if (!range) {
        range = document.createRange();
      }
      ...
      for (i = 0, l = texts.length;currentNode = texts[i];++i) {
        ...
        for (j = 0, m = currentNode.data.length;j < m;++j) {
          ...
          rect = X_TextRange_getCorrectRect(isTextField, range);
          if (sumup(i + j === 0, isLast, j, rect.top | 0, rect.bottom | 0)) {
            range.setEnd(isLast ? currentNode : lastTextNode, 1 + (isLast ? j : lastOffset));
            range.setStart(startLineTextNode, startLineOffset);
            return{range:range, rect:X_TextRange_getCorrectRect(isTextField, range), from:startLineFrom, to:startLineFrom + (range + "").length};
            return range;
          }
      ...
      for (i = 0;i < l;++i) {
        isLast = i + 1 === l;
        rect = X_TextRange_getCorrectRect(isTextField, range);
        if (sumup(i === 0, isLast, i, rect.top, rect.bottom)) {
          range.moveStart("character", startLineFrom - i);
          if (!isLast) {
            range.moveEnd("character", -1);
          }
          return{range:range, rect:X_TextRange_getCorrectRect(isTextField, range), from:startLineFrom, to:startLineFrom + range.text.length};
        }
        range.moveEnd("character", 1);
        range.moveStart("character", 1);
      }
  ...
  LineRangeFromLast = function(xnode, isTextField, from, to) {
    ...
    return result && {_xnode:xnode, _isTextField:isTextField, _result:result, _getRawRange:LineRangeFromLast_getRawRange, "getOffset":X_TextRange_getOffset, "getRect":!isTextField || X_TextRange_ieRange ? X_TextRange_getRect : null, "move":X_TextRange_move, "text":isTextField ? X_TextRange_text : null, "select":X_TextRange_select};
  };
  function LineRangeFromLast_getRawRange(range, xnode, isTextField, from, to) {
    var elm = xnode["_rawObject"], texts, i, l, is1stNode, currentNode, j, m, is1st, isLast, rect, minY, lines = 0, lastTextNode, lastOffset, startLineFrom, endLineTextNode, endLineOffset;
    if (!isTextField && X_TextRange_w3cRange) {
      if (!range) {
        range = document.createRange();
      }
      ...
      for (i = l = texts.length;currentNode = texts[--i];) {
        ...
        for (j = m = currentNode.data.length;j;--j) {
          ...
          rect = X_TextRange_getCorrectRect(isTextField, range);
          if (sumup(is1st, isLast, j, rect.top | 0, rect.bottom | 0)) {
      ...
      for (;i;--i) {
        isLast = i === 1;
        rect = X_TextRange_getCorrectRect(isTextField, range);
        if (sumup(i === l, isLast, i - 1, rect.top, rect.bottom)) {
          !isLast && range.moveStart("character", 1);
          range.moveEnd("character", endLineOffset - i + 1);
          if (isLast) {
            --i;
          }
          return{range:range, rect:X_TextRange_getCorrectRect(isTextField, range), from:i, to:i + range.text.length};
        }
        range.moveStart("character", -1);
        range.moveEnd("character", -1);
      }
  ...
  LineRangeFromPoint = function(xnode, isTextField, x, y) {
    ...
    return result && {_xnode:xnode, _isTextField:isTextField, _result:result, _getRawRange:LineRangeFromPoint_getRawRange, "getOffset":X_TextRange_getOffset, "getRect":!isTextField || X_TextRange_ieRange ? X_TextRange_getRect : null, "move":X_TextRange_move, "text":isTextField ? X_TextRange_text : null, "select":X_TextRange_select};
  };
  function LineRangeFromPoint_getRawRange(range, xnode, isTextField, x, y) {
    var elm = xnode["_rawObject"], texts, i, l, currentNode, isLastNode, j, m, isLast, rect, hit, maxB, lines = [], lastTextNode, lastOffset, totalOffset = 0, startLineTextNode, startLineOffset, startLineFrom, currentLineStartTextNode, currentLineStartOffset, currentLineStartFrom;
    if (!isTextField && X_TextRange_w3cRange) {
      if (!range) {
        range = document.createRange();
      }
    ...
    function sumup(is1st, last, offset, y, b) {
      ...
      if (cr || last) {
        if (cr && startLineTextNode) {
          isLast = false;
        }
        if (startLineTextNode) {
          return true;
        } else {
          if (cr || !currentLineStartTextNode) {
            currentLineStartTextNode = currentNode;
  ...
  X.buildTimeStamp = "2018/03/28 23:17:04.92";
  window["X"] = X;
